...
def main():
    ...
    with tab2:
        st.markdown('<p class="tab-header">Crime Density Analysis</p>', unsafe_allow_html=True)
        
        if len(filtered_df) == 0:
            st.warning("No data available for the selected filters")
        else:
            try:
                # Clean data for density mapping
                density_df = filtered_df.dropna(subset=['Latitude', 'Longitude'])
                density_df = density_df[(density_df['Latitude'] != 0) & (density_df['Longitude'] != 0)]
                ...
                if len(density_df) > 0:
                    ...
                    fig = px.density_mapbox(
                        ...
                        zoom=10,
                        height=650,
                        title=f"Crime Density Distribution ({len(density_df):,} incidents)",
                        mapbox_style="open-street-map"
                    )
                    fig.update_layout(margin={"r":0,"t":50,"l":0,"b":0})
                    st.plotly_chart(fig, use_container_width=True)
                    
                    # Density by time periods
                    col1, col2 = st.columns(2)
                    
                    with col1:
                        # Day vs Night density
                        day_crimes = density_df[density_df['Hour'].between(6, 18)]
                        if len(day_crimes) > 10:
                            fig_day = px.density_mapbox(
                                day_crimes, lat="Latitude", lon="Longitude",
                                radius=15, zoom=10, height=400,
                                title="Day Time Crime Density (6AM-6PM)",
                                mapbox_style="open-street-map"
                            )
                            fig_day.update_layout(margin={"r":0,"t":50,"l":0,"b":0})
                            st.plotly_chart(fig_day, use_container_width=True)
                    
                    with col2:
                        # Night density
                        night_crimes = density_df[~density_df['Hour'].between(6, 18)]
                        if len(night_crimes) > 10:
                            fig_night = px.density_mapbox(
                                night_crimes, lat="Latitude", lon="Longitude",
                                radius=15, zoom=10, height=400,
                                title="Night Time Crime Density (6PM-6AM)",
                                mapbox_style="open-street-map"
                            )
                            fig_night.update_layout(margin={"r":0,"t":50,"l":0,"b":0})
                            st.plotly_chart(fig_night, use_container_width=True)
                    
                    # Density by crime type (top 4)
                    if 'Primary Type' in density_df.columns:
                        top_crimes = density_df['Primary Type'].value_counts().head(4).index.tolist()
                        
                        if len(top_crimes) > 0:
                            st.markdown("### Crime Type Density Maps")
                            cols = st.columns(2)
                            
                            for i, crime_type in enumerate(top_crimes):
                                crime_data = density_df[density_df['Primary Type'] == crime_type]
                                
                                if len(crime_data) >= 10:
                                    fig = px.density_mapbox(
                                        crime_data, lat="Latitude", lon="Longitude",
                                        radius=12, zoom=10, height=350,
                                        title=f"{crime_type} Density ({len(crime_data)} cases)",
                                        mapbox_style="open-street-map"
                                    )
                                    fig.update_layout(margin={"r":0,"t":50,"l":0,"b":0})
                    ...
                    st.warning("No valid coordinate data available for density mapping")
                    
            except Exception as e:
                st.error(f"Error creating density maps: {e}")
    with tab3:
